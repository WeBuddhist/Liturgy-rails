...
DEFAULT_TRACK_PREAMBLE = (
    "A canonical Tibetan liturgical text from the ཞལ་འདོན་ཕྱོགས་བསྒྲིགས། collection of "
    "the most widely recited prayers, praises, aspirations and dhāraṇīs. The "
    "blocks below are being translated in order."
)
...
def main():
    ...
    # FORK(liturgy-rails): upstream stripped every non-ASCII character, so a
    # Tibetan filename slugged to "" and fell back to "text" -- all 95 notes
    # in this corpus would have overwritten one another as text-en.md. Keep
    # the source stem when it has no Latin characters to slug.
    _base = meta.get("title_in_english") or src_path.stem
    slug = re.sub(r"[^a-z0-9]+", "-", _base.lower()).strip("-")[:40]
    if not slug:
        slug = re.sub(r'[/\\:*?"<>|]', "", src_path.stem).strip() or "text"
    ...
    def context_for(batch):
        ...
        prior = [r for r in ledger if r["block_id"] not in ids]
