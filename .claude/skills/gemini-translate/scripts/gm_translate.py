...
python3 4-SYSTEM/scripts/stamp_metadata.py --ids --track-meta --titles --track "{track}"
```
...
def build_request(batch, ctx, reference, args, parity_n=None):
    ...
    system = args.style.strip() + FORMAT_RULES
...
def main():
    ...
    def do_render():
        ...
        prov = {
            ...
            "fields": {
                ...
                "line_parity_failures": sum(1 for r in ordered if r.get("line_parity") is False),
            },
            "warning": (
        ...
        dm.render(out_md, units, ordered, meta, args, src_rel, prov=prov)
        return ordered
    ...
    print(f"target     : {args.lang} ({args.lang_tag})   model={args.model}"
          f"   thinking={args.thinking or 'default'}")
    ...
    def context_for(batch):
        ...
        prior = [r for r in ledger
                 if r["block_id"] in order and order[r["block_id"]] < first_pos]
    ...
    ordered = do_render()
    done_now = len({r["block_id"] for r in ledger})
