...
python3 4-SYSTEM/scripts/stamp_metadata.py --ids --track-meta --titles --track "3-TRANSFORMATIONS/Translations/Gemini/hi"
```
...
| 2026-09-06 | Full corpus, `gm_corpus.py --lang` (model `gemini-3.1-pro-preview`, thinking default) | 94/94 texts, 1892/1892 blocks, 194 calls in the final resumed run, 0 line-parity failures, 0 failed blocks |
...
| 2026-09-06 | Names pinned (`gm_names.py --propose`; 38 glossary entries) and drifting blocks re-run (`--rerun`, two passes) | 7 block(s) of 489 name occurrences still lack the pinned form after the boundary-aware check — epithets or longer names containing the key, listed by `gm_names.py --check` |
