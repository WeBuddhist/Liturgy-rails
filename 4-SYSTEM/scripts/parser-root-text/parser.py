...
# FORK(liturgy-rails): upstream allowed at most ONE hyphen, so a 3-level
# id like ^1-2-3 never matched and its block was dropped as "no reference".
...
def _extract_blocks(body):
    blocks = []
    # FORK(liturgy-rails): upstream split blocks on blank lines only, so a
    # heading not separated by one was absorbed into the adjacent block --
    # leaking raw '^id' and '##' markup into content, with no warning. A
